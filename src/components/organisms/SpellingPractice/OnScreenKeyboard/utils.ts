...
/**
 * Gets the right highlighted keys for each specific scenario
 * and returns an array of the characters
 * @param char the characters to be highlighted
 */
export const getHighlightedKeys = (char: string) => {
    ...
    if (char === SPACE) return retArray;
        ...
        SHIFT_PUNC_MARKS.includes(char)
    )
        retArray.push(SHIFT);
    if (ACCENTED_CHARS.includes(char.toLocaleLowerCase()))
        retArray.push(ACCENT);
    retArray.push(
